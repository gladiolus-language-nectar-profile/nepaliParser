...
//	Final tags;
       ...
       Mymatching mm = new Mymatching(Nouns.length+nKarkas, Nouns.length, nKarkas);
       //t.getFrame(Verbs);
        try{
	  	if( decesion == 1 ) 	{
			...
			nonouns = Nouns.length;
		      ...
		      // vibhakti = t.findVibhakti(sentwords1,morphs, Vibhakti, nonouns,nouns);
		       ...
		       int [][][] finalmatching1 = new int[MaxSG][Nouns.length][Verbs.length];
		       System.out.println("N.len="+Nouns.length+"    nkarkas="+nKarkas);
		   //    nKarkas = 3;
//		       Nouns.length = 2;
		       ...
		       mm.main1(Nouns.length+ntKarkas, ntKarkas, krkseq);
		       for(int l = 0 ; l < Verbs.length ; l++){
				       subgraphs = t.findsubGraphs(edges, nKarkas, Nouns.length, l, indexkarana);
					finalmatching = t.findfinalmatching(subgraphs, 9, Nouns.length, nKarkas, indexkarta, indexkarana, NP);
		       }
...
//    	int psnverb;
	...
	String[][]  prsce = new String[8][3];
	String[][] ktk = new String[8][3];
	String[][] vbkt = new String[8][3];
	vb[0] = vbr;
	//	ReadXMLFile xm = new ReadXMLFile();
    	System.out.println("getTAM");
    	System.out.println("verb="+vbr);
   // 	f2.readFile1(wordnetfile);
        f2.main1(vb,preNN,wordnetfile);
 					    ...
 					    try{
 					        for(int i = 0 ; i < tvbt.length ; i++ ){
 					        	...
 					        	//for(int l = 0 ; l < tvbt.length ; l++){
 					        		if(count == 0 || Actualvibhaktilist[t].equals(tvbt[i])){
									System.out.println("hi");
 					        			for(int k = 0; k < trows ; k++){
	 					        			System.out.println("tg["+0+"]["+k+"]["+1+"]="+tg[0][k][1] +"   count="+count); 					        			
	 					        			if((tg[0][k][1]) == 1){
	 					        				count = 1;
	 					        			}
	 					        		}
 ...
 /*   public String getPresence(int inx , ReadXMLFile f3){
   ...
   public static void displayString1(String outputName5, String[][][] wds, int wdslen){
    ...
    try{
	        ...
	        fout.write("\n\t  KARTA(k1)      \tKARMA(k2)     \tKARANA(k3)     \tSAMPRADAN(K4)   \tAPADAN(K5)  \tSAMBANDH(K6) \tLOCATION(K2P)");
...
//	   	   		fout.write(nn[l]);
    ...
    public void getFrame(String[] verb){
    	...
    	ReadXMLFile xm = new ReadXMLFile();
    	xm.main1(verb, preNN, wordnetfile);
...
//	for(int u1 = 0 ; u1 < nsgr ; u1++){
   	...
   	//}
	if(flg == true ){
		System.out.println("\t  KARTA(k1)   \tKARMA(k2)\t  KARANA(k3)\t  SAMPRADAN(K4) \tAPADAN(k5)   \tSAMBANDH(K6)  \tADHIKARAN(K7)    \tLOCATION(K2P)");
	   	   	for(int l = 0 ; l < rows ; l++){
