...
class Mymatching1 
{
	 ...
	 static int[] per= new int[100000];
	 static int t = 0;
	 static int danger = 0;
	 static int factorial = 0;

	 Mymatching1(){};
	 Mymatching1(int V1, int M1, int N1, int dan){
	 	M = M1;
	 	N = N1;
	 	V=V1;
	 	danger = dan;
	 ...
	 void Matching(int V, Graph g){
		...
		int index = 0;
		while(hasaugmentingPath(g, mate, index, V)){
			 int t = -1;
		         for (int v = 0; v < V; v++) {
	 ...
	 boolean hasaugmentingPath(Graph g, int[] mat, int ind, int V){
	 	//int V  = 0;
	 	//this.V = V;
//	 	this.V = g.V;
		if(ind == 0)
		...
		while (!queue.isEmpty()) {
		        ...
		        //System.out.println("w="+w+"  marked["+w+"]="+marked[w]);
	                ...
	                if (isResidualGraphEdge(v, w) && !marked[w]) {
            	    	...
            	    	//System.out.println("Hi its Mymatching1.java");
            	    }
        	}
    ...
    public int[] getmatch(int V){
    	//System.out.println("final matching:"+ V);
    	for(int i = 0 ; i < V; i++){
    ...
    public static int[][][] getIncidenceMat(int l, int m, int n, int[] finmat, int danger){
    	
    	int[][][] inc = new int[l+1][m][m+n];
    	//System.out.println("danger="+danger);
    	if(danger == 1){
 		   	Random rand = new Random();
    		int rn = rand.nextInt(m+n-1);
    		finmat[m-1] = (int)(Math.random()*((m+n-1) - (m) + 1)+ m);
    		//System.out.println("Random number ="+finmat[m-1]);
       	}
    	//for(int i = 0 ; i < l ; i++){
    	//System.out.println(" *** Incidence Matrix : "+n+"   m="+m);
    	for(int j = 0 ; j < m ; j++){
    			for(int k = 0 ; k < n ; k++){
    				inc[l][j][k] = 0;
    			}
    		...
    		for(int j = 0 ; j < m ; j++){
    			for(int k = 0 ; k < (m+n) ; k++){
    				...
    				if(finmat[j] != -1){
    					inc[l][j][finmat[j]] = 1;
    					//System.out.print(" inc["+l+"]["+j+"]["+finmat[j]+"] "+inc[l][j][finmat[j]]);
    				}
    			}
    			//System.out.println();
    		}
    	...
    	for(int j = 0 ; j < m ; j++){
    			for(int k = m ; k < (m+n) ; k++){
    				//System.out.print("    "+inc[l][j][k]);
    				//IncidenceMatrix[l][j][k] = inc[l][j][k];
    			}
    			//System.out.println();
    	}
    	return inc;
...
//                if(i == r) index++;
    ...
    public static int[][][] main1(int V, int N, int[] B, int dg) 
    { 
        // create the graph given in above figure 
	      // int V = 5; 
	      //  int N = 3;
	      int minindex = Math.min(V-N,N);
	        //System.out.println("number of vertices are "+V+" elements in part B are"+N);
	        int[] finmate = new int[V];
	        int maxmatchEdges = 0;
	        int fac = factorial(N);
	        factorial = fac;
	        int[][] permutation = new int[fac][N];
	        int[][] prm = new int[2000][N];
	        int[] B1 = new int[fac];
	        int[][][] incidenceMatrix1 = new int[1][V-N][N];
	        //int 
	        int[][][] incidenceMatrix = new int[fac][V-N][N];
	        int[][][] finalincidenceMatrix = new int[fac][V-N][N];
	        ...
	        try{
		        for(int l = 0 ; l < fac ; l++){
			        for(int l1 = 0 ; l1 < N  ; l1++){	
			        	...
			        	//if(l < N)      	        
				       		//System.out.print("     newmm permu["+l+"]["+l1+"]="+permutation[l][l1]);
			        	k++;
			        }
			        //System.out.println();
			}
		...
		//System.out.println("B.length = "+B.length+"    N="+N);
		for(int l = 0 ; l < fac ; l++){
		        for(int l1 = 0 ; l1 < N  ; l1++){	
			        if(permutation[l][l1] == 0){
			        	for(int s = 0 ; s < N ; s++){
			        		for(int s1 = 0 ; s1 < N ; s1++){
				        		if(permutation[l][s1] != B[s]){
								permutation[l][l1] = s ;
							}		        			
			        		}
			        	}
		        		...
		        		//permutation[l][l1] = per[k];        	        
		        	////System.out.print("     **permu["+l+"]["+l1+"]="+permutation[l][l1]);
		        	k++;
		        }
		        //System.out.println();
		}
		...
		int multiplier = 0;
	       /* for(int p = 0 ; p < fac ; p++ ){
	        	//for(int q = 0 ; q < N ; q++){
	        		//q=0;
	        		//count = 0;
	        		////System.out.println("q="+q+"      count="+count+"   permup["+p+"]["+0+"]="+permutation[p][0]+"   B["+q+"]="+B[q]);
	        		//if(q < N){
//	        			//System.out.println("q="+q+"      count="+count+"   permup["+p+"]["+q+"]="+permutation[p][q]+"   B["+q+"]="+B[q]);
		        		if(permutation[p][0] == B[q]){
		        			if(count < 250){
			        			count++;
			        			...
			        			try{
			        				for(int r = 0 ; r < N ; r++){
				        				prm[200*multiplier + count][r] = permutation[p][r];
				        				//System.out.println("**count = "+count+"    multi="+(250*multiplier + count)+" prm="+prm[250*multiplier + count][r]);
				        			}
				        		}
				        		catch(ArrayIndexOutOfBoundsException e){}
		        			}
		...
		int cnt = 0;
	        for(int l = 0 ; l < fac ; l++){
	        	//if((0 <= l && l< 700) || ((fac-100)<=l && l<fac)){
	        		//System.out.println(" New permutation l = "+l);
	                 	Mymatching1 m = new Mymatching1(); 
  			        Mymatching1.Graph g = m.new Graph(V);
  			        maxmatchEdges = 0;
  			        try{
		        		for(int m2 = 0 ; m2 < N ; m2++){
			        	  //System.out.println("prmutation["+l+"]["+m2+"] : "+permutation[l][m2]);
				      	  addEdge(g,0,permutation[l][m2]);
				      	  addEdge(g,1,permutation[l][m2]);
				      	  addEdge(g,2,permutation[l][m2]);
				      	  
					}
				}
				catch(ArrayIndexOutOfBoundsException e){}
			        m.Matching(V,g);	
			        //System.out.println("  finmat = "+finmate.length);
			        finmate = m.getmatch(V);
			 	printGraph(g);	
			 	
			 	incidenceMatrix1 = getIncidenceMat(cnt,V-N,N, finmate,dg);
			 	//System.out.print(" welcome incidence matrix  ");
			 	//for(int s = 0 ; s < cnt ; s++){
			 		for(int i = 0 ; i < (V-N) ; i++){
			 			for(int j = (V-N) ; j < V ; j++){
			 				incidenceMatrix[cnt][i][j-(V-N)] = incidenceMatrix1[cnt][i][j];
			 				//System.out.print("  "+incidenceMatrix1[cnt][i][j]);
			 ...
			 //	}
			 	...
			 	//System.out.println("V="+V);
			 	 System.out.print("Max matching: ");
			        ...
			        Mymatching1.Graph g1 = m.new Graph(maxmatchEdges);
	        		for (int v = 0; v < V; v++) {
			        	if(finmate[v] != -1){
...
//					        else
	        ...
	        //System.out.println("My matching incidence matrix : V="+V+"    N="+N);
	        int pos =0;
	        for(int s = 0 ; s < cnt ; s++){
	        	
		 	pos = getUniqueIncMatrix(s,(V-N),N,incidenceMatrix);
		 	
		 	//System.out.println("pos = "+pos);
		 	if(pos != -1){
		 		for(int i = 0 ; i < (V-N) ; i++){
			 		for(int j = 0 ; j < N ; j++){
			 			finalincidenceMatrix[s][i][j] = incidenceMatrix[s][i][j];
			 		}
			 	}
		 	}
		}
	        	...
	        	for(int s = 0 ; s < cnt ; s++){
			 	for(int i = 0 ; i < (V-N) ; i++){
			 		for(int j = 0 ; j < N ; j++){
			 			System.out.print("  "+finalincidenceMatrix[s][i][j]);
			 		}
			 		System.out.println();
			 	}
			 	System.out.println();
			}
	        return finalincidenceMatrix;
    ...
    public static int getUniqueIncMatrix(int current, int rows, int col, int[][][] im ){
    	int flag = 0;
    	int count = 0;
    	try{
    	if(current != 0){
	    	...
	    	count = 0;
	    		for(int j = 0 ; j < rows ; j++){
	    			for(int k = 0 ; k < col ; k++){
	    				if(im[current][j][k] != im[i][j][k]){
	    				////System.out.println("im["+current+"]["+j+"]["+k+"]"+im[current][j][k]);
	    					flag = 1;
	    					break;
	    					//count++;
	    					//return current;
	    					...
	    					if(count == rows*col){
	    						//System.out.println("Repeat Incidence Matrix no:"+current);
		    					return -1;
		    				}
	    				}
	    			}
